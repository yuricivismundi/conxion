              ...
              <div className="space-y-4">
                {filteredComposeConnections.length > 0 ? (
                  <div className="space-y-2">
                    <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-cyan-200/80">Connections</p>
                    {filteredComposeConnections.map((target) => (
                      <button
                        key={target.connectionId}
                        type="button"
                        onClick={() => onSelectConnection(target)}
                        className="flex w-full items-center gap-3 py-2.5 px-1 text-left hover:opacity-80 transition-opacity"
                      >
