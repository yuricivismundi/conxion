-- Allow any joined attendee (going/waitlist) to send invites, regardless of guests_can_invite flag.
...
begin
  ...
  if v_has_sender_id and v_has_recipient_id and v_has_status then
    v_sql := format(
      'insert into public.event_invitations (id, event_id, sender_id, recipient_id, note, status, created_at)
       values (%L, %L, %L, %L, %L, %L, now())
       on conflict (event_id, recipient_id) do update
         set sender_id  = excluded.sender_id,
             note       = excluded.note,
             status     = excluded.status,
             created_at = excluded.created_at
       returning id',
      v_invitation_id, p_event_id, v_me, p_recipient_id, p_note, 'pending'
    );
  elsif v_has_sender_id and v_has_recipient_id then
    v_sql := format(
      'insert into public.event_invitations (id, event_id, sender_id, recipient_id, note, created_at)
       ...
       on conflict (event_id, recipient_id) do update
         set sender_id  = excluded.sender_id,
             ...
             created_at = excluded.created_at
       returning id',
      v_invitation_id, p_event_id, v_me, p_recipient_id, p_note
    );
  else
    v_sql := format(
      'insert into public.event_invitations (id, event_id, inviter_id, invitee_id, note, created_at)
       values (%L, %L, %L, %L, %L, now())
  ...
  execute v_sql into v_invitation_id;
