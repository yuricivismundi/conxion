...
export default function NotificationsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 px-6 text-center">
      <span className="material-symbols-outlined text-[48px] text-slate-500">notifications_off</span>
      <div className="space-y-1">
        <p className="text-base font-semibold text-white">Couldn&apos;t load notifications</p>
      ...
      <div className="flex gap-3">
        ...
        <Link href="/" className="rounded-full bg-white/5 px-5 py-2.5 text-sm font-semibold text-slate-300 hover:bg-white/10">Home</Link>
      </div>
    </div>
  );
}
