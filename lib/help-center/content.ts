...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    body: [
      "Starter is the free entry plan. It lets you discover social dancers, teachers, organizers, DJs, and artists, send 10 connection requests per month, start up to 10 new conversations per month (chat activations), send 5 hosting offers per month, create 1 trip per month (up to 5 trip requests), create 2 events per month, hold up to 5 private groups total, send 5 activity requests per month, offer hosting, and use Dance Tools.",
      "Verified is a one-time trust upgrade. It is not a subscription and it does not replace Plus. Verified adds a badge, unlocks requesting hosting (up to 10 requests per month), unlocks teacher or artist profile, and opens service inquiries. Everything in Starter still applies after verifying.",
      "Plus is the monthly visibility and usage plan. It raises your account to 60 connection requests per month, 30 chat activations per month, 10 hosting offers per month, 10 trip requests per month (create up to 5 trips), 5 events per month, up to 10 private groups total, 15 activity requests per month, and adds 3 profile photos alongside 2 showcase videos. It also includes private mode to hide from Discover, and your trips and events appear before free-plan members. Verified and Plus stay separate products because one is trust access and the other is recurring growth access.",
    ],
  ...
  {
    slug: "how-teacher-booking-works",
    title: "How does booking a private class work?",
    summary: "Teachers set open availability slots. Students pick a date and slot, send a request, and the teacher accepts or declines.",
    category: "using-conxion",
    updatedAt: "2026-05-08",
    body: [
      "To use the teacher booking flow you need a Verified account — this is required to unlock teacher and service inquiry access.",
      "Teachers: set your open availability slots in your teacher profile settings (Edit Profile → Teacher Profile → Availability). Each slot has a date, start time, and end time. Slots marked available will appear to visitors on your teacher profile.",
      "Students: visit a teacher's profile page and open the booking section. Select a date from the calendar, pick a time slot, add an optional note about your level and goals, then tap Send booking request.",
      "The teacher receives a notification and can accept or decline the request from their Bookings manager. Accepting a booking marks the slot as taken and automatically declines any competing requests for the same slot.",
      "Bookings are limited to dates within the next 3 months. Past dates and already-accepted slots are filtered out automatically.",
      "You cannot book yourself — if you visit your own teacher profile, the booking flow shows a notice that visitors use this page to request sessions.",
      "After acceptance, coordinate timing and location over your ConXion chat thread. The accepted booking also generates a calendar link so you can save it to your device calendar.",
    ],
    related: ["when-do-i-need-verification-for-hosting", "how-starter-verified-and-plus-work", "how-connections-and-activities-work"],
  },
  {
    slug: "groups-events-and-activity-limits",
    title: "What are the limits for groups, events, and activity requests?",
    summary: "Groups have a total cap per plan. Events and activity requests reset monthly.",
    category: "billing-plans",
    updatedAt: "2026-05-08",
    body: [
      "Private groups: Starter lets you hold up to 5 private groups at the same time. Plus raises that to 10. Unlike trips and events, this is a total cap — not a monthly creation limit. If you leave or archive a group, that slot becomes available again.",
      "Events: Starter lets you create up to 2 events per calendar month. Plus raises that to 5 per month. Both plans support public and request-access event types.",
      "Activity requests: Starter includes 5 activity requests per month (practice sessions, social dances, classes, collaborations, etc.). Plus raises that to 15 per month. Activity requests are separate from connection requests.",
      "Trip requests: Starter includes 5 trip join requests per month and lets you create 1 trip per month. Plus raises trip requests to 10 per month and lets you create up to 5 trips per month.",
      "All monthly counters (events, activity requests, trip requests, connection requests, chat activations) reset on the 1st of each calendar month. Chat windows already open continue until their 30-day window expires — the reset only affects how many new conversations you can start.",
    ],
    related: ["how-starter-verified-and-plus-work", "when-do-i-need-verification-for-hosting"],
  },
  {
  ...
  {
    slug: "request-linked-chat-windows",
    title: "How do request-linked chat windows work?",
    summary: "When a hosting or activity request is accepted, a temporary chat window opens automatically — without using one of your monthly chat activations.",
    category: "using-conxion",
    updatedAt: "2026-04-15",
    body: [
      "When a hosting request or an activity request is accepted, ConXion grants a temporary chat window automatically. This window lets both members coordinate in the thread tied to that request without consuming one of their monthly chat activations.",
      "Scheduled state: before the window opens, the thread shows a note with the date chat becomes available. You can still see past messages, but you cannot send new ones until the window is active.",
      "Active state: once the window is open, both members can send messages freely. A note in the thread shows the expiry date so you know how long the window lasts.",
      "Expired state: after the window closes, sending new messages requires a chat activation. That uses one of your monthly activation slots, just like starting any other new conversation.",
      "Window timing for hosting requests: if the trip start date is within 14 days of acceptance, the window opens immediately. If the trip is further away, the window opens 14 days before the start date. The window closes at the end of the trip end date, or at the end of the start date if no end date was set.",
      "Window timing for date-based activity requests (Practice, Social Dance, Event, Travelling, and similar): the same 14-day rule applies to the activity start time. The window closes at the end of that day.",
      "Window timing for non-date activities: if a request has no start date or time attached, the window opens immediately on acceptance and expires 48 hours later.",
      "One window per request: each hosting or activity request creates at most one chat entitlement. Cancelling the request before acceptance cancels the entitlement. Accepting a new request of the same type on the same thread creates a fresh window.",
      "What to do after the window expires: if you still want to coordinate or send messages, either member can activate the conversation normally. This uses one monthly chat activation and keeps the thread open for 30 days from that activation.",
      "Important: the request-linked window is separate from conversation activation. Activating a conversation does not cancel or replace an open window — both can coexist on the same thread.",
    ],
    related: ["how-connections-and-activities-work", "when-do-i-need-verification-for-hosting", "travel-and-hosting-safety-basics"],
  },
  {
