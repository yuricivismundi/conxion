...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    body: [
      "Starter is the free entry plan. It lets you discover social dancers, teachers, organizers, DJs, and artists, send 10 connection requests per month, keep up to 10 active chat threads, send 5 hosting offers per month, create 1 trip per month (up to 5 trip requests), create 2 events per month, hold up to 5 private groups total, send 5 activity requests per month, offer hosting, and use Dance Tools.",
      ...
      "Plus is the monthly visibility and usage plan. It raises your account to 60 connection requests per month, 30 active chat threads, 10 hosting offers per month, 10 trip requests per month (create up to 5 trips), 5 events per month, up to 10 private groups total, 15 activity requests per month, and adds 3 profile photos alongside 2 showcase videos. It also includes private mode to hide from Discover, and your trips and events appear before free-plan members. Verified and Plus stay separate products because one is trust access and the other is recurring growth access.",
    ],
  ...
  },
  {
    ...
    body: [
      ...
      "All monthly counters (events, activity requests, trip requests, connection requests) reset at the start of each calendar month.",
    ],
  ...
  },
  {
    ...
    summary: "When a hosting or activity request is accepted, a temporary chat window opens automatically — without using a normal active chat slot.",
    category: "using-conxion",
    updatedAt: "2026-04-15",
    body: [
      "When a hosting request or an activity request is accepted, ConXion grants a temporary chat window automatically. This window lets both members coordinate in the thread tied to that request without consuming a normal active chat slot for either person.",
      ...
      "Expired state: after the window closes, sending new messages requires a normal conversation activation. That activation uses the regular active chat quota for both parties, just like any other connection thread.",
      ...
      "What to do after the window expires: if you still want to coordinate or send messages, either member can activate the conversation normally. This uses a normal active chat slot and removes the time limit.",
